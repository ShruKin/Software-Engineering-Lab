...
if (isset($_REQUEST["status"])) {
  $query = "update course set status='" . $_REQUEST['status'] . "' where cid='" . $_REQUEST["id"] . "'";
  $result = mysqli_query($con, $query);
  header("location:courses.php");
}
?>
      ...
      <tr>
        ...
        <td>
          <?php if ($fetch->status == 1) { ?>
            <a href="?status=0&id=<?php echo $fetch->cid ?>">Active</a>
          <?php } else if ($fetch->status == 0) { ?>
            <a href="?status=1&id=<?php echo $fetch->cid ?>">Inactive</a>
          <?php } ?>
        </td>
